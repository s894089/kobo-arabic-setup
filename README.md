...
السكربتات مكتوبة بـ bash، وعلى ويندوز هذا يعني **WSL** — نظام Ubuntu حقيقي
تشحنه مايكروسوفت داخل ويندوز. ليس PowerShell، وليس Git Bash (فهو بلا `rsync`،
والمثبّت مبنيّ على `rsync`). هذا هو المسار الوحيد المدعوم على ويندوز في هذا
الدليل، وهو الذي جُرِّب وأُصلحت مشكلاته.

**إعداد لمرة واحدة — نحو عشر دقائق وإعادة تشغيل واحدة**

١. افتح قائمة ابدأ، اكتب `PowerShell`، انقر بالزر الأيمن ← **Run as
   administrator**. الصق:
...
<div dir="rtl">
   ...
   دعه ينتهي، ثم **أعد تشغيل الحاسوب**.

٢. بعد إعادة التشغيل تُفتح نافذة بعنوان **Ubuntu** من تلقاء نفسها (وإلا فافتحها
   من قائمة ابدأ). تطلب منك **اسم مستخدم** و**كلمة مرور**. هذه كلمة مرور
   *لينكس*، مستقلة عن ويندوز — سيطلبها المثبّت مرة واحدة لاحقًا، فاحفظها. لا
   يظهر شيء أثناء كتابتها؛ هذا طبيعي.

٣. في نافذة Ubuntu نفسها، ثبّت الأدوات الثلاث:
...
**في كل مرة تثبّت أو تحدّث**

١. وصّل الكوبو، افتح قفله، اضغط **Connect** على شاشته.
٢. انتظر حتى يُظهر مستكشف ويندوز **KOBOeReader (D:)** — قد يختلف الحرف. إن لم
   يُظهره المستكشف، فلن يراه شيء مما يلي.
٣. افتح **Ubuntu** من قائمة ابدأ — *لا* PowerShell، *لا* Git Bash.
٤. نفّذ خطوات التثبيت في القسم التالي كما هي.

المثبّت يجد الكوبو عبر ويندوز نفسه، فلا يهم أوصّلته قبل فتح Ubuntu أم بعده. وإن
لم يكن WSL قد رأى القرص بعد، يقول لك السكربت ذلك، ويعرض الأمر الوحيد الذي
سينفّذه، ويطلب كلمة مرور لينكس لتنفيذه. هذا متوقَّع.

**إن حدث خطأ**

| ما تراه | معناه | ما تفعله |
|---|---|---|
| `No Kobo found` | ويندوز لم يركّب الجهاز | تأكد أن المستكشف يُظهر **KOBOeReader**. وإلا: افصل، أعد التوصيل، اضغط **Connect**، انتظر ١٠ ثوانٍ. |
| `No Kobo found` والمستكشف يُظهره كـ `D:` | WSL لا يرى حرف القرص | `KOBO_MOUNT=D: ./install.sh` — بحرفك أنت. وإن لم ينفع: أغلق Ubuntu، افصل، أعد التوصيل، افتح Ubuntu من جديد. |
| `[sudo] password for …` | السكربت يركّب القرص | اكتب كلمة مرور لينكس (لا تظهر أثناء الكتابة) ثم Enter. |
| `bash\r: No such file or directory` | نسخة قديمة استُنسخت بنهايات أسطر ويندوز | `git pull` ثم `git checkout -- .` — المستودع يثبّت نهايات الأسطر الآن. |
| `rsync: command not found` | تخطّيت الخطوة ٣ من الإعداد | نفّذها. |
| `Permission denied` عند `./install.sh` | فقد صلاحية التنفيذ | استعمل `bash install.sh`. |

وعلى أي نظام، إن كان الجهاز في مكان غير معتاد، أشِر إليه مباشرة:
...
إن لم يُعثر على جهازك، راجع جدول الأعطال تحت نظامك أعلاه (لمستخدمي ويندوز:
جدول WSL). وكحلّ أخير:

</div>

```bash
KOBO_MOUNT=/path/to/kobo ./install.sh      # على ويندوز يعمل أيضًا KOBO_MOUNT=D:
```
